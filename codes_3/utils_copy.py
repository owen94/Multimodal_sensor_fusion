...
def error_ratio(data1,data2,indi_matrix = None,missingrate = 0):
    if missingrate == 0:
        print('This is the no missing case')
        a = numpy.sum((data1 - data2)**2)
        b = numpy.sum(data1**2)
    else:
        a = numpy.sum(((data1 - data2)*(1 - indi_matrix))**2)
        b = numpy.sum((data1*(1 - indi_matrix))**2)
    return numpy.sqrt(a)/numpy.sqrt(b)
...
def load_test_data(data,numMod,missing_rate):
    ...
    for i in range(numMod):
        test_list[i], teststats_list[i] = normActiv(raw1[:,i*visible_size_Mod:(i+1)*visible_size_Mod])
    datasets = numpy.concatenate(test_list, axis=1)


    return raw,datasets,teststats_list,indi_matrix

def load_sae_test_data(data,numMod,missing_rate,indi_matrix):

    print('Loading sae testing data................')
    numMod = numMod
    raw = numpy.load(data)
    raw = raw[2400:,:]
    print(raw.shape)
    ...
    return raw,datasets,teststats_list
