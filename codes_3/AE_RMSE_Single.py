...
class AE(object):
    ...
    def __init__(
        ...
        """
        Initialize the dA class by spec ifying the number of visible units (the
        ...
        numMod: The input modality of the input data set

        missing: A boolean variable to indicate whether the
        ...
        # self.indi_matrix = theano_rng.binomial(size=input.shape, n=1,
        #                                 p=1 - .9,
        #                                 dtype=theano.config.floatX)

        self.indi_matrix = T.cast(indi_matrix, 'int32')

        self.n_visible = n_visible
        self.n_hidden = n_hidden

        norm_indi = T.sum(self.indi_matrix, axis = 0)
        #norm_indi = norm_indi.reshape(theano.shared(1),self.n_visible)
        ...
        if self.numMod > 1:
            ...
            multi_sparsity = numpy.ones((1,self.n_hidden))
            multi_sparse_weight = numpy.ones((1,self.n_hidden))
    ...
    def get_cost_updates(self,learning_rate):
        ...
        if self.missing:
            ...
            #cost_part1 = 0.5*T.sum(((1*residue)**2))\
                         #+0.5*(T.sum((self.W1*self.G)**2)+T.sum((self.W2*self.G.T)**2))

            mean_activation = T.dot(self.indi_matrix.T, y)/self.norm_indi

            #mean_activation = T.mean(y,axis=0)
            #
            # a = mean_activation/self.multi_sparsity
            #
            # b = T.log(a)
            #
            # d = (1-self.multi_sparsity)/(1-mean_activation)
            #
            # c = T.log((1-self.multi_sparsity)/(1-mean_activation))
            #
            # KL = self.multi_sparsity*T.log(self.multi_sparsity)/mean_activation\
            #      + (1-self.multi_sparsity)*T.log((1-self.multi_sparsity)/(1-mean_activation))

            #mean_activation =T.sum(T.dot(self.indi_matrix.T, y),axis = 0)/self.norm_indi


            KL = mean_activation*(T.log(mean_activation) -T.log(self.multi_sparsity))\
                 + (1-mean_activation)*(T.log(1-mean_activation) - T.log(1-self.multi_sparsity))

            A = T.sum(T.log(1-mean_activation))
...
def test_AE(data='',validationdata= '', param_list= None,missing = True, missing_rate = 0.4,learning_rate=0.08, training_epochs= 1000,
            batch_size=3000, output_folder='dA_plots'):
...
def get_reconstructed_input(hidden,W2,b2,G,missing = False):
    ...
    if missing:
        ...
        return tanh(numpy.dot(hidden, W2*G.T) + b2)
